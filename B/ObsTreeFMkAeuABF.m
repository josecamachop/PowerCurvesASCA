...
% Branch Full Model: X = m + A + B + AB (changes due to unballanceness)
...
function e = ObsTreeFMkAeuABF(Xm,D,parglmo,vt,vc)
        ...
        for j = 1 : n_perm     
            ...
            SSQ_time(1 + j) = (sum(sum((D1p(:,parglmo.factors{2}.Dvars)*B(parglmo.factors{2}.Dvars,:)).^2))/dfB) / (sum(sum((D1p(:,parglmo.interactions{1}.Dvars)*B(parglmo.interactions{1}.Dvars,:)).^2))/dfAB);
