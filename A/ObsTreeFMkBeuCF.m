...
function e = ObsTreeFMkBeuCF(Xm,D,parglmo,vp)
        ...
        SSQ_class(1) = (sum(sum((D1(:,parglmo.factors{1}.Dvars)*B(parglmo.factors{1}.Dvars,:)).^2))/dfA) / (sum(sum((D1(:,parglmo.factors{3}.Dvars)*B(parglmo.factors{3}.Dvars,:)).^2))/dfC);
        ...
        for j = 1 : n_perm
            ...
            SSQ_class(1 + j) = (sum(sum((D1(:,parglmo.factors{1}.Dvars)*B(parglmo.factors{1}.Dvars,:)).^2))/dfA) / (sum(sum((D1(:,parglmo.factors{3}.Dvars)*B(parglmo.factors{3}.Dvars,:)).^2))/dfC);
