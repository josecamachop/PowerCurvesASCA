...
function e = ObsTreeFMF(Xm,D,parglmo)
        ...
        D1 = D;
        B1 = pinv(D1'*D1)*D1'*Xm;
        Xa = D1(:,parglmo.factors{1}.Dvars)*B1(parglmo.factors{1}.Dvars,:);
        X_residuals = Xm - D1*B1;
        
        Tdf = size(Xm,1)-1;      
        Rdf = Tdf - dfA - dfB - dfAB -dfC;
        
        SSQ_ind(1) = (sum(sum((Xa).^2))/dfA) / (sum(sum((X_residuals).^2))/Rdf);
        ...
        for j = 1 : n_perm
            ...
            SSQ_ind(1 + j) = (sum(sum((Xa).^2))/dfA) / (sum(sum((X_residuals).^2))/Rdf);
