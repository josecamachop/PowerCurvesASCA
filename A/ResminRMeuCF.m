...
function e = ResminRMeuCF(Xm,D,parglmo,vp)
        ...
        D1 = D(:,[1 parglmo.factors{2}.Dvars]);
        B = pinv(D1'*D1)*D1'*Xm;
        X_residuals = Xm - D1*B;
        
        D2 = D(:,[parglmo.factors{1}.Dvars parglmo.interactions{1}.Dvars]);
        B = pinv(D2'*D2)*D2'*X_residuals;
        
        Xa = D2(:,1:length(parglmo.factors{1}.Dvars))*B(1:length(parglmo.factors{1}.Dvars),:);
        Xab = D2(:,length(parglmo.factors{1}.Dvars)+1:end)*B(length(parglmo.factors{1}.Dvars)+1:end,:);
       
        SSQ_class(1) = (sum(sum(Xa.^2))/dfA) / (sum(sum(Xab.^2))/dfAB);
        ...
        for j = 1 : n_perm
            ...
            SSQ_class(1 + j) = (sum(sum(Xa.^2))/dfA) / (sum(sum(Xab.^2))/dfAB);
