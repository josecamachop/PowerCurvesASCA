function parglmo = parglm2(X, F, interactions, center, n_perm)
...
% according to Andreson and Ter Braak. It also uses the sum of squares of the first 2PCs 
% as permutation index.
%
% parglmo = parglm2(X, F)   % minimum call
% parglmo = parglm2(X, F, interactions, center, n_perm)   % complete call
%
%
% INPUTS:
%
...
for f = 1 : n_factors
    ...
    [P,T] = pca_pp(preprocess2D(parglmo.factors{f}.matrix,center),1:min(2,rank(parglmo.factors{f}.matrix)));
    SSQ_factors(1,f) = sum(sum((T*P').^2));
end

% Interactions
for i = 1 : n_interactions
    parglmo.interactions{i}.matrix = D(:,parglmo.interactions{i}.Dvars)*B(parglmo.interactions{i}.Dvars,:);
    [P,T] = pca_pp(preprocess2D(parglmo.interactions{i}.matrix,center),1:min(2,rank(parglmo.interactions{i}.matrix )));
    SSQ_interactions(1,i) = sum(sum((T*P').^2));
end
...
for j = 1 : n_perm
    ...
    for f = 1 : n_factors
        ...
        [P,T] = pca_pp(preprocess2D(factors{f}.matrix,center),1:min(2,rank(factors{f}.matrix)));
        SSQ_factors(1 + j,f) = sum(sum((T*P').^2));
    end
    
    % Interactions
    for i = 1 : n_interactions
        interacs{i}.matrix = D(:,parglmo.interactions{i}.Dvars)*B(parglmo.interactions{i}.Dvars,:);
        [P,T] = pca_pp(preprocess2D(interacs{i}.matrix,center),1:min(2,rank(interacs{i}.matrix)));
        SSQ_interactions(1 + j,i) = sum(sum((T*P').^2));
    end
