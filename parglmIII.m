function parglmo = parglmIII(X, F, interactions, center, n_perm)
...
% according to Andreson and Ter Braak. It also uses the type III sum of squares  
% as permutation index.
%
% parglmo = parglmIII(X, F)   % minimum call
% parglmo = parglmIII(X, F, interactions, center, n_perm)   % complete call
%
%
% INPUTS:
%
...
for f = 1 : n_factors
    ...
    D1 = D;
    D1(:,parglmo.factors{f}.Dvars) = [];
    B1 = pinv(D1'*D1)*D1'*X;
    X_residuals1 = X - D1*B1;
    SSQ_factors(1,f) = sum(sum(X_residuals1.^2)) - sum(sum(X_residuals.^2));
end
...
for i = 1 : n_interactions
    ...
    D1 = D;
    D1(:,parglmo.interactions{i}.Dvars) = [];
    B1 = pinv(D1'*D1)*D1'*X;
    X_residuals1 = X - D1*B1;
    SSQ_interactions(1,i) = sum(sum(X_residuals1.^2)) - sum(sum(X_residuals.^2));
end
...
for j = 1 : n_perm
    ...
    for f = 1 : n_factors
        ...
        D1 = D;
        D1(:,parglmo.factors{f}.Dvars) = [];
        B1 = pinv(D1'*D1)*D1'*X(perms, :);
        X_residuals1 = X(perms, :) - D1*B1;
        SSQ_factors(1 + j,f) = sum(sum(X_residuals1.^2)) - sum(sum(X_residuals.^2));
    end
    ...
    for i = 1 : n_interactions
        ...
        B1 = pinv(D1'*D1)*D1'*X(perms, :);
        X_residuals1 = X(perms, :) - D1*B1;
        SSQ_interactions(1 + j,i) = sum(sum(X_residuals1.^2)) - sum(sum(X_residuals.^2));
    end
