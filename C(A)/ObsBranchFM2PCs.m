...
%
% Permute: Raw Data
% Branch Full Model: X = m + A + C(A) (changes due to unballanceness)
% Constrain: None
% Ex Units: unit samples
% Statistic: 2PCs of SSE
%
...
function e = ObsBranchFM2PCs(Xm,D,parglmo)
		...
		Xc = D1(:,parglmo.factors{3}.Dvars)*B(parglmo.factors{3}.Dvars,:);    
        [P,T] = pca_pp(Xc,1:min(2,rank(Xc)));
        SSQ_ind(1) = sum(sum((T*P').^2));
