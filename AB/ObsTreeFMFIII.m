...
function e = ObsTreeFMFIII(Xm,D,parglmo)
        ...
        D2 = D(:,1:(parglmo.interactions{1}.Dvars(1)-1));
        B2 = pinv(D2'*D2)*D2'*Xm;
        Xab = D1*B1 - D2*B2;
        ...
        SSQ_interactions(1) = (sum(sum((Xab).^2))/dfAB) / (sum(sum((X_residuals).^2))/Rdf);
        ...
        for j = 1 : n_perm
            ...
            SSQ_interactions(1 + j) = (sum(sum((Xab).^2))/dfAB) / (sum(sum((X_residuals).^2))/Rdf);
