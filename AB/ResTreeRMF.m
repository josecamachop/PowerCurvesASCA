...
function e = ResTreeRMF(Xm,D,parglmo)
        ...
        Tdf = size(Xm,1)-1;      
        Rdf = Tdf - dfA - dfB - dfAB;
        
        SSQ_interactions(1) = (sum(sum((D2*B).^2))/dfAB) / (sum(sum((Xr).^2))/Rdf);
        ...
        for j = 1 : n_perm
            ...
            Xr = X_residuals(perms, :) - D2*B;

            SSQ_interactions(1 + j) = (sum(sum((D2*B).^2))/dfAB) / (sum(sum((Xr).^2))/Rdf);
